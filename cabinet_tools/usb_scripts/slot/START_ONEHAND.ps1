...
function Test-DriveReady {
    param([string]$Letter)
    try {
        $d = [IO.DriveInfo]::new("${Letter}:\")
        return [bool]$d.IsReady
    } catch {
        return $false
    }
}

function Ensure-GoldClubVolume {
    if ((Test-DriveReady 'G') -and (Test-Path -LiteralPath 'G:\Bootstrap.exe')) {
        return
    }
    $ud = 'C:\Platform\Security\UnlockerDisk.exe'
    if (-not (Test-Path -LiteralPath $ud)) {
        Write-Step 'WARN G: not ready and UnlockerDisk.exe missing'
        return
    }
    Write-Step 'G: not mounted (BitLocker locked) - starting UnlockerDisk'
    Start-Process -FilePath $ud -WindowStyle Hidden -ErrorAction SilentlyContinue | Out-Null
    $sw = [Diagnostics.Stopwatch]::StartNew()
    while ($sw.Elapsed.TotalSeconds -lt 45) {
        if ((Test-DriveReady 'G') -and (Test-Path -LiteralPath 'G:\Bootstrap.exe')) {
            Write-Step 'G: GOLDCLUB unlocked'
            return
        }
        Start-Sleep -Seconds 2
    }
    Write-Step 'WARN G: still locked after UnlockerDisk'
}

function Find-GoldclubRoot {
    param([string]$Preferred)
    Ensure-GoldClubVolume
    if ($Preferred -and (Test-Path -LiteralPath $Preferred)) {
        return (Resolve-Path -LiteralPath $Preferred).Path
    }
    # Live slot GOLDCLUB is G:\ (C:\goldclub is a junction to G:\). Do not
    # Join-Path a letter that is not ready - that throws DriveNotFound.
    $cands = New-Object System.Collections.Generic.List[string]
    if (Test-DriveReady 'C') { $cands.Add('C:\goldclub'); $cands.Add('C:\Goldclub') }
    if (Test-DriveReady 'G') { $cands.Add('G:\'); $cands.Add('G:\Goldclub') }
    ...
    throw "Goldclub root not found (G: BitLocker locked or missing). Tried C:\goldclub and G:\"
}
...
foreach ($procName in @("Bootstrap", "Start-SlotGameWatch", "BiOS2", "OneHand", "game-start")) {
...
Write-Step 'Kicking GoldClub-Ensure-HwStack (SYSTEM) so HWSubsys actually starts'
cmd /c 'schtasks /Run /TN "GoldClub-Ensure-HwStack" /I' | Out-Null
$hwDeadline = [Diagnostics.Stopwatch]::StartNew()
$hwUp = $false
while ($hwDeadline.Elapsed.TotalSeconds -lt 40) {
    $hw = Get-Service -Name 'GoldClub Hardware Subsystem' -ErrorAction SilentlyContinue
    if (-not $hw) { $hw = Get-Service -DisplayName 'GoldClub Hardware Subsystem' -ErrorAction SilentlyContinue }
    if ($hw -and $hw.Status -eq 'Running') { $hwUp = $true; break }
    Start-Sleep -Milliseconds 500
}
if ($hwUp) {
    Write-Step 'HWSubsys Running - extra 8s settle'
    Start-Sleep -Seconds 8
} else {
    Write-Step 'WARN HWSubsys not Running after Ensure-HwStack - Bootstrap/OneHand will likely HW-timeout'
