...
def test_default_live_target_sweeps_image_drives_after_c() -> None:
    from config_scanner.live_push import _LOCAL_LIVE_CANDIDATES
    ...
    from config_scanner.live_push import _local_candidate_path
...
def test_this_pc_and_111_default_resolve_local_root_first(tmp_path: Path) -> None:
    from config_scanner.live_push import (
        THIS_PC_LIVE_TARGET,
    ...
    target, note = resolve_live_load_target(
        THIS_PC_LIVE_TARGET, prefer_local=True, local_candidates=cands
    )
    ...
    # Without a local tree the default stays .111 (leave that).
...
def test_this_pc_without_goldclub_explains_instead_of_cmdkey(tmp_path: Path) -> None:
    missing = tmp_path / "Goldclub"
    root, err = prepare_live_goldclub(str(missing))
    assert root is None
    assert "on this PC" in err
    assert "slot\\themes" in err
    assert "Browse" in err
    assert "cmdkey" not in err
    assert err.strip(), "the Load warning must never be blank"
