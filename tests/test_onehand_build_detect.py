...
def test_all_utf16_values_after_skips_earlier_numeric_product_version() -> None:
    blob = (
        _utf16_pair("ProductVersion", "2.0.1+RC2")
        + b"\x00" * 16
        + _utf16_pair("ProductVersion", "Debug")
    )
    assert _all_utf16_values_after(blob, "ProductVersion") == ("2.0.1+RC2", "Debug")
...
def test_debug_sku_from_versioninfo_past_first_4mb(tmp_path: Path) -> None:
    ...
    exe.write_bytes(prefix + _version_info_blob(ProductVersion="Debug", FileVersion="2.0.1.0"))
    info = detect_onehand_build(gold)
    assert info is not None
    assert info.configuration == "Debug"
...
def test_debug_sku_from_fileversion_string_when_productversion_numeric(tmp_path: Path) -> None:
    gold = tmp_path / "Goldclub"
    slot = gold / "slot"
    slot.mkdir(parents=True)
    exe = slot / "OneHand.exe"
    exe.write_bytes(
        ...
        + b"\x00\x00"
        + "Release".encode("utf-16le")
        + _version_info_blob(ProductVersion="2.0.1+RC2", FileVersion="Debug")
    )
    assert onehand_exe_is_debug_sku(exe) is True
    info = detect_onehand_build(gold)
    assert info is not None
    assert info.configuration == "Debug"
    assert "2.0.1" in (info.version or "")
...
def test_prefers_slot_onehand_over_root_release_copy(tmp_path: Path) -> None:
    ...
    (gold / "OneHand.exe").write_bytes(
        b"MZ" + _version_info_blob(ProductVersion="2.1.0", FileDescription="OneHand Release")
    )
    (slot / "OneHand.exe").write_bytes(
        b"MZ" + _version_info_blob(ProductVersion="Debug", FileVersion="2.0.1.0")
    )
